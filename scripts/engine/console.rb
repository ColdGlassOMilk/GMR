...
module Console
  ...
  class << self
    ...
    def draw
      ...
      set_color(15, 20, 30, 240)
      draw_rect(0, y, width, @height)
      
      # Top border (subtle)
      set_color(40, 50, 70, 255)
      draw_rect(0, y, width, 1)
      
      # Bottom border (highlighted)
      set_color(60, 120, 180, 255)
      draw_rect(0, y + @height - 2, width, 2)
    ...
    def draw_output(y, width)
      ...
      (start_idx...end_idx).each do |i|
        ...
        draw_text(text, padding, draw_y, font_size)
        draw_y += line_height
      end
    end
    ...
    def draw_input_line(y, width)
      ...
      # Input background
      set_color(25, 30, 45, 255)
      draw_rect(0, input_y - 4, width, 28)
      
      # Prompt
      prompt = ">> "
      prompt_color = @open ? [100, 200, 255] : [100, 100, 100]
      set_color(*prompt_color)
      draw_text(prompt, padding, input_y, font_size)
      
      prompt_width = measure_text(prompt, font_size)
      text_x = padding + prompt_width
      
      # Input text
      set_color(255, 255, 255)
      draw_text(@input, text_x, input_y, font_size)
      
      # Cursor
      if @open && @cursor_blink < 1
        cursor_x = text_x + measure_text(@input[0...@cursor_pos], font_size)
        set_color(255, 255, 255, 200)
        draw_rect(cursor_x, input_y, 2, font_size + 2)
      end
      
      # Show history index if navigating
      if @history_index >= 0
        indicator = "[#{@command_history.length - @history_index}/#{@command_history.length}]"
        indicator_width = measure_text(indicator, 12)
        set_color(80, 80, 100)
        draw_text(indicator, width - indicator_width - padding, input_y + 1, 12)
      end
    end
    ...
    def draw_scrollbar(y, width)
      ...
      # Track
      set_color(40, 45, 60, 150)
      draw_rect(bar_x, bar_y, 4, bar_height)
      ...
      set_color(80, 120, 180, 200)
      draw_rect(bar_x, thumb_y, 4, thumb_height)
    end
    
    def color_for_type(type)
      case type
      when :input  then [120, 180, 255]      # Blue - user input
      when :result then [120, 255, 150]      # Green - successful result
      when :error  then [255, 100, 100]      # Red - error
      when :warn   then [255, 200, 100]      # Yellow - warning
      when :info   then [160, 160, 180]      # Gray - info
      else              [200, 200, 200]      # Default
      end
    end
    ...
    def execute_command
      ...
      if cmd_lower == "clear" || cmd_lower == "cls"
        ...
        if new_height >= 100 && new_height <= screen_height - 50
          @height = new_height
          @target_y = @open ? 0 : -@height
          @history << { type: :info, text: "Console height set to #{new_height}" }
        else
          @history << { type: :error, text: "Height must be between 100 and #{screen_height - 50}" }
        end
      else
    ...
    def show_help
      ...
      @history << { type: :result, text: "  screen_width" }
      @history << { type: :result, text: "  toggle_fullscreen" }
      @history << { type: :result, text: "  set_window_title 'New Title'" }
    end
