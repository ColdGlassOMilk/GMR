...
def draw
  ...
  Graphics.draw_rect(5, 5, 805, 135, [0, 0, 0, 180])  # Semi-transparent black background

  # Draw text over the background
  cam_pos = @camera.target
  Graphics.draw_text("FPS: #{fps}", 10, 10, 30, :yellow)
  Graphics.draw_text("Objects: #{@objects.size}", 10, 38, 26, :cyan)
  Graphics.draw_text("Target: (#{@camera_target.x.round}, #{@camera_target.y.round})", 10, 65, 24, :white)
  Graphics.draw_text("Camera: (#{cam_pos.x.round}, #{cam_pos.y.round}) Zoom: #{@camera.zoom.round(2)}", 10, 90, 24, :white)
  Graphics.draw_text("Controls: WASD/Arrows=Pan | Wheel=Zoom | R=Reset | +/-=Add/Remove", 10, 115, 22, [200, 200, 200, 255])
