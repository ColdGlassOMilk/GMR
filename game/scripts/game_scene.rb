...
class GameScene < GMR::Scene
  ...
  def on_resize(width, height)
    ...
    # Camera offset is in pixels too (center of viewport)
    @camera.offset = Mathf::Vec2.new(@screen_width / 2.0, @screen_height * CAMERA_OFFSET_Y_PERCENT)
  end
  ...
  def setup_camera
    @camera = Graphics::Camera.new(
      viewport_size: Mathf::Vec2.new(@screen_width, @screen_height),
  ...
  def setup_shaders
    ...
    @shaders = [
      ...
      Graphics::Shader.load(fragment: "shaders/glitch.fs", surface_mode: true),
  ...
  def set_shader_uniforms(shader, name)
    res_x = @screen_width.to_f
    res_y = @screen_height.to_f
