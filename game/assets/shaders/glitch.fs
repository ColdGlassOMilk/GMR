...
void main()
{
    ...
    float r = texture2D(texture0, uv + vec2(splitAmount, 0.0)).r;
    float g = texture2D(texture0, uv).g;
    float b = texture2D(texture0, uv - vec2(splitAmount, 0.0)).b;
    ...
    if (blockNoise > 1.0 - intensity * 0.05) {
        vec2 blockUV = uv + vec2(rand(vec2(time)) - 0.5, rand(vec2(time * 2.0)) - 0.5) * 0.1;
