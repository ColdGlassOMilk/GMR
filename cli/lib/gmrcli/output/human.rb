...
module Gmrcli
  module Output
    ...
    # Used when --human flag is passed (human mode is opt-in)
    # Provides colored, styled console output for interactive use
    class Human
      class << self
        def pastel
        ...
        # Check if human mode is enabled (inverse of machine mode)
        def enabled?
          !Emitter.enabled?
        end
